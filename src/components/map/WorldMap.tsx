...
function TripMarker({ trip, isSelected, zoom, onClick }: TripMarkerProps) {
  ...
  return (
    <Marker coordinates={[trip.longitude!, trip.latitude!]}>
      <g
      ...
      >
        ...
        {isReady && (
          <ComposableMap
            ...
            projectionConfig={{ scale: 175, center: [15, 5] }}
          ...
          >
            ...
            >
              {/* Ocean background */}
              <rect x={-2000} y={-2000} width={4000} height={4000} fill="#0e0c08" />

              <Geographies geography={GEO_URL}>
                {({ geographies }: { geographies: GeoType[] }) =>
                  geographies.map((geo) => {
                    const numericId = parseInt(String(geo.id), 10);
                    const isVisited = visitedNumericCodes.has(numericId);
                    return (
                      <Geography
                        key={geo.rsmKey}
                        geography={geo}
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-1.5 z-10">
          <button
            ...
            style={{ background: 'rgba(21,18,9,0.85)', border: '1px solid rgba(181,98,30,0.3)', color: '#C9B98A' }}
            aria-label="Zoom in"
          >
        ...
        <div
          ...
          style={{ background: 'rgba(10,8,4,0.75)', padding: '6px 8px', borderRadius: '8px', border: '1px solid rgba(181,98,30,0.2)' }}
        >
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full inline-block" style={{ background: '#C9B98A', boxShadow: '0 0 6px #C9B98A' }} />
            Upcoming
          </span>
        </div>
        ...
        {!selectedTrip && mappableTrips.length > 0 && (
          <div
            ...
            style={{ background: 'rgba(10,8,4,0.6)', padding: '4px 8px', borderRadius: '6px' }}
          >
        ...
        {/* Selected trip popup */}
        {selectedTrip && (
          <TripPopup trip={selectedTrip} onClose={() => setSelectedTrip(null)} />
        )}
      </div>
    </div>
  );
