...
export default function AdminBudgetPage() {
  ...
  const [togglingCell, setTogglingCell] = useState<string | null>(null); // `${memberId}:${itemId}`
  ...
  useEffect(() => {
    if (tab === 'members' && !costItemsLoaded) {
      loadCostData();
    }
  }, [tab, costItemsLoaded, loadCostData]);
  ...
  const handleToggleAssignment = async (memberId: string, itemId: string) => {
  ...
  const handleSaveNote = async (memberId: string, itemId: string) => {
    setSavingNote(true);
    const existing = costAssignments.find((a) => a.member_id === memberId && a.cost_item_id === itemId);
    try {
      const h = { ...(await getAuthHeader()), 'Content-Type': 'application/json' };
  ...
  const COST_ITEM_ICONS: Record<string, React.ReactNode> = {
    'international flights': <Plane className="w-3.5 h-3.5" />,
    'travel insurance': <Shield className="w-3.5 h-3.5" />,
    'visa fees': <FileText className="w-3.5 h-3.5" />,
  };

  const getCostItemIcon = (name: string) => {
    return COST_ITEM_ICONS[name.toLowerCase()] ?? <DollarSign className="w-3.5 h-3.5" />;
  };
  ...
  const [expForm, setExpForm] = useState({
    ...
    category_id: '', paid_by_type: 'group_kitty', paid_by_member: '', paid_by_label: '', account_source_id: 'westpac_choice', notes: '',
  });
  ...
  const [incomeForm, setIncomeForm] = useState({ description: '', amount_aud: '', income_date: new Date().toISOString().split('T')[0], category: 'other', account_source_id: 'westpac_choice', notes: '' });
  ...
  const tabs = [
    ...
    { key: 'export' as Tab, label: 'Tax Export', icon: Download },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-brand-cream/50 text-sm">Income · Expenses · Budget · Tax Export</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      {pendingAccountAssignmentCount > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 rounded-lg border border-brand-tan/30 bg-brand-dark-grey">
          <p className="text-sm text-brand-cream/80">
            {pendingAccountAssignmentCount} account assignment{pendingAccountAssignmentCount === 1 ? '' : 's'} queued
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setPendingAccountAssignments({})}
      ...
      {overview && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          ...
          <div className="rounded-xl border border-brand-tan/20 bg-brand-dark-grey px-4 py-3">
            <p className="text-[11px] uppercase tracking-wide text-brand-cream/45">Unassigned Transactions</p>
            <p className={`mt-1 text-xl font-semibold ${(unassignedExpenses.length + unassignedIncome.length) > 0 ? 'text-amber-400' : 'text-green-400'}`}>
              {unassignedExpenses.length + unassignedIncome.length === 0 ? '✓ All assigned' : unassignedExpenses.length + unassignedIncome.length}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-brand-cream/50 mt-0.5">Calculated from all recorded transactions assigned to each account.</p>
            </div>
          </div>

          {/* Balance cards — ledger net per account */}
          {(() => {
            const wcNet = accountFlowSummary.find((r) => r.id === 'westpac_choice')?.net ?? 0;
            const wlNet = accountFlowSummary.find((r) => r.id === 'westpac_life')?.net ?? 0;
            const ppNet = accountFlowSummary.find((r) => r.id === 'paypal')?.net ?? 0;
            const cards = [
              { label: 'Westpac Choice 524337', balance: wcNet, icon: Building2, color: 'border-brand-tan/20', textColor: 'text-brand-cream' },
              { label: 'Westpac Life 253840',   balance: wlNet, icon: Building2, color: 'border-brand-tan/20', textColor: 'text-brand-cream' },
              { label: 'PayPal',                balance: ppNet, icon: Wallet,    color: 'border-amber-600/20', textColor: 'text-amber-300'  },
              { label: 'Total Funds',           balance: wcNet + wlNet + ppNet, icon: DollarSign, color: 'border-green-600/30', textColor: 'text-green-400' },
            ];
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                {cards.map(({ label, balance, icon: Icon, color, textColor }) => (
                  <div key={label} className={`rounded-xl border ${color} bg-brand-dark-grey px-5 py-4`}>
                    ...
                    <p className={`text-2xl font-bold ${textColor}`}>{fmt(balance)}</p>
                    <p className="text-[10px] mt-1 text-brand-cream/30">from recorded transactions</p>
                  </div>
                ))}
              </div>
            );
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={label} className="rounded-lg bg-amber-900/10 border border-amber-600/20 px-4 py-3 text-center">
                    ...
                    <p className="font-bold text-amber-300">{fmt(wlNet * (interestRatePa / 100) / 12 * months)}</p>
                  </div>
                ))}
              </div>
              {(() => {
                ...
                const projected = wlNet * (interestRatePa / 100) / 12 * monthsElapsed;
      ...
      {tab === 'income' && (
        <div className="space-y-4">
          {unassignedIncome.length > 0 && (
            <div className="flex items-start gap-3 p-3 bg-amber-900/20 border border-amber-600/30 rounded-lg">
              <AlertTriangle className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" />
              <p className="text-sm text-amber-400">
                <span className="font-semibold">{unassignedIncome.length} income {unassignedIncome.length === 1 ? 'entry' : 'entries'} have no account assigned.</span>{' '}
                Assign an account in the Account column below so these appear correctly in ledger and export reports.
              </p>
            </div>
          )}
          <div className="flex items-center justify-between">
        ...
        <div className="space-y-4">
          {unassignedExpenses.length > 0 && (
            <div className="flex items-start gap-3 p-3 bg-amber-900/20 border border-amber-600/30 rounded-lg">
              <AlertTriangle className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" />
              <p className="text-sm text-amber-400">
                <span className="font-semibold">{unassignedExpenses.length} {unassignedExpenses.length === 1 ? 'expense has' : 'expenses have'} no account assigned.</span>{' '}
                Assign an account in the Account column below so these are correctly attributed in the ledger and tax reports.
              </p>
            </div>
          )}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          MEMBERS TAB — per-member cost responsibility tracker
      ══════════════════════════════════════════════════════════════════════ */}
      {tab === 'members' && (
        <div className="space-y-6">
          {/* Header */}
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 className="text-lg font-bold text-brand-cream">Member Cost Tracker</h2>
              <p className="mt-0.5 text-sm text-brand-cream/55">
                Track which costs each member is handling themselves vs. covered by group deposits.
                A <span className="font-semibold text-brand-tan">✓ Self-funded</span> tick means the member is sorting that item independently.
              </p>
            </div>
            <button
              onClick={() => { setAddingCostItem(true); setTimeout(() => newCostItemRef.current?.focus(), 50); }}
              className="flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-tan text-brand-black font-semibold text-sm hover:bg-brand-tan/90 transition-colors"
            >
          ...
          {/* Add cost item form */}
          {addingCostItem && (
            <div className="rounded-xl border border-brand-tan/30 bg-brand-dark-grey p-4 space-y-3">
              <p className="text-sm font-semibold text-brand-cream">New Cost Item</p>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                <div className="flex-1 space-y-1">
                  <label className="text-xs text-brand-cream/60 uppercase tracking-wide">Name *</label>
                  <input
                    ref={newCostItemRef}
                    value={newCostItemName}
                    onChange={(e) => setNewCostItemName(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') handleAddCostItem(); if (e.key === 'Escape') setAddingCostItem(false); }}
                    placeholder="e.g. International Flights"
                    className="w-full rounded-lg border border-brand-tan/30 bg-brand-black px-3 py-2 text-sm text-brand-cream placeholder:text-brand-cream/30 focus:outline-none focus:ring-2 focus:ring-brand-tan"
                  />
                </div>
                <div className="flex-1 space-y-1">
                  <label className="text-xs text-brand-cream/60 uppercase tracking-wide">Description (optional)</label>
                  <input
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={savingCostItem || !newCostItemName.trim()}
                    className="px-4 py-2 rounded-lg bg-brand-tan text-brand-black font-semibold text-sm hover:bg-brand-tan/90 transition-colors disabled:opacity-50"
                  >
                    {savingCostItem ? 'Saving…' : 'Save'}
                  </button>
                  <button
                    onClick={() => { setAddingCostItem(false); setNewCostItemName(''); setNewCostItemDesc(''); }}
                    className="px-3 py-2 rounded-lg border border-brand-tan/20 text-brand-cream/60 text-sm hover:text-brand-cream hover:border-brand-tan/40 transition-colors"
                  >
                    Cancel
                  </button>
                </div>
          ...
          {/* Loading state */}
          {!costItemsLoaded && (
            <div className="flex items-center gap-3 py-12 justify-center text-brand-cream/40">
              <RefreshCw className="w-5 h-5 animate-spin" />
              <span className="text-sm">Loading…</span>
            </div>
          )}

          {/* Empty state — no cost items yet */}
          {costItemsLoaded && costItems.length === 0 && !addingCostItem && (
            <div className="rounded-xl border border-dashed border-brand-tan/20 p-12 text-center">
              <Users className="w-10 h-10 text-brand-tan/30 mx-auto mb-3" />
              <p className="text-brand-cream/60 font-medium">No cost items defined yet</p>
              <p className="text-sm text-brand-cream/40 mt-1">
                Add items like &ldquo;International Flights&rdquo;, &ldquo;Travel Insurance&rdquo;, or &ldquo;Visa Fees&rdquo; to start tracking.
              </p>
              <button
                onClick={() => { setAddingCostItem(true); setTimeout(() => newCostItemRef.current?.focus(), 50); }}
                className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-tan/20 text-brand-tan font-semibold text-sm hover:bg-brand-tan/30 transition-colors"
              >
                <Plus className="w-4 h-4" />
                Add First Cost Item
              </button>
          ...
          {/* Matrix grid */}
          {costItemsLoaded && costItems.length > 0 && tripMembers.length > 0 && (
            <div className="space-y-3">
              {/* Cost item chips / legend */}
              <div className="flex flex-wrap gap-2">
                {costItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-1.5 rounded-full border border-brand-tan/25 bg-brand-black/40 px-3 py-1 text-xs text-brand-cream/70">
                    {getCostItemIcon(item.name)}
                    <span>{item.name}</span>
                    <button
                      onClick={() => handleDeleteCostItem(item.id)}
                      className="ml-1 text-brand-cream/30 hover:text-red-400 transition-colors"
              ...
              {/* Scrollable table */}
              <div className="overflow-x-auto rounded-xl border border-brand-tan/20">
                <table className="w-full min-w-[640px] text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-brand-tan/15 bg-brand-black/50">
                      <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-brand-cream/50 w-44">
                        Member
                      </th>
                      {costItems.map((item) => (
                        <th key={item.id} className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-brand-cream/50 min-w-[140px]">
                  ...
                  <tbody>
                    {tripMembers.map((member, rowIdx) => {
                      const displayName = getMemberListName(member);
                      return (
                        <Fragment key={member.id}>
                          <tr className={`border-b border-brand-tan/10 transition-colors ${rowIdx % 2 === 0 ? 'bg-brand-dark-grey' : 'bg-brand-black/20'} hover:bg-brand-black/40`}>
                            {/* Member name */}
                            <td className="px-4 py-3 font-medium text-brand-cream whitespace-nowrap">
                              {displayName}
                            </td>
                            {/* One cell per cost item */}
                            {costItems.map((item) => {
                              const cellKey = `${member.id}:${item.id}`;
                              const assignment = costAssignments.find(
                                (a) => a.member_id === member.id && a.cost_item_id === item.id
                              );
                              const isSelfFunded = assignment?.is_self_funded ?? false;
                              const hasNote = !!assignment?.notes;
                              const isToggling = togglingCell === cellKey;
                              const isEditingNote = editingNoteCell === cellKey;

                              return (
                                <td key={item.id} className="px-4 py-3 text-center align-top">
                                  <div className="flex flex-col items-center gap-1.5">
                                    {/* Toggle button */}
                                    <button
                                      onClick={() => handleToggleAssignment(member.id, item.id)}
                                      disabled={isToggling}
                                      title={isSelfFunded ? 'Self-funded — click to unmark' : 'Not self-funded — click to mark'}
                                      className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-all ${
                                        isSelfFunded
                                          ? 'bg-green-900/40 border border-green-600/50 text-green-400 hover:bg-green-900/60'
                                          : 'bg-brand-black/40 border border-brand-tan/15 text-brand-cream/35 hover:border-brand-tan/35 hover:text-brand-cream/60'
                                      } disabled:opacity-50`}
                                    >
                                      {isToggling ? (
                                      ...
                                      {isSelfFunded ? 'Self-funded' : 'Group covers'}
                                    </button>
                                    ...
                                    {isEditingNote ? (
                                      <div className="w-full space-y-1">
                                        <textarea
                                          ...
                                          className="w-full rounded border border-brand-tan/30 bg-brand-black px-2 py-1 text-xs text-brand-cream placeholder:text-brand-cream/30 focus:outline-none focus:ring-1 focus:ring-brand-tan resize-none"
                                        />
                                        <div className="flex gap-1 justify-end">
                                          <button
                                            onClick={() => handleSaveNote(member.id, item.id)}
                                            disabled={savingNote}
                                            className="px-2 py-0.5 rounded text-xs bg-brand-tan text-brand-black font-semibold disabled:opacity-50"
                                          >
                                            {savingNote ? '…' : 'Save'}
                                          </button>
                                          <button
                                            onClick={() => setEditingNoteCell(null)}
                                            className="px-2 py-0.5 rounded text-xs border border-brand-tan/20 text-brand-cream/50 hover:text-brand-cream"
                                          >
                                            Cancel
                                          </button>
                                        </div>
                                      ...
                                      <button
                                        onClick={() => { setEditingNoteCell(cellKey); setNoteDraft(assignment?.notes ?? ''); }}
                                        className={`text-xs transition-colors max-w-[130px] truncate ${hasNote ? 'text-brand-tan/80 hover:text-brand-tan' : 'text-brand-cream/20 hover:text-brand-cream/50'}`}
                                        title={hasNote ? assignment!.notes! : 'Add a note'}
                                      >
                                        {hasNote ? assignment!.notes! : '+ note'}
                                      </button>
                                    )}
                                  </div>
                                </td>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 pt-2">
                {costItems.map((item) => {
                  const selfFundedCount = costAssignments.filter(
                    (a) => a.cost_item_id === item.id && a.is_self_funded
                  ).length;
                  const total = tripMembers.length;
                  return (
                    <div key={item.id} className="rounded-lg border border-brand-tan/15 bg-brand-black/30 px-4 py-3">
                      <div className="flex items-center gap-1.5 mb-1.5 text-brand-cream/60">
                        {getCostItemIcon(item.name)}
                        <span className="text-xs font-semibold truncate">{item.name}</span>
                      </div>
                      <p className="text-xl font-bold text-brand-cream">{selfFundedCount}<span className="text-sm font-normal text-brand-cream/40">/{total}</span></p>
                      <p className="text-xs text-brand-cream/40 mt-0.5">self-funded</p>
                      {/* Mini progress bar */}
                      <div className="mt-2 h-1 rounded-full bg-brand-black/60 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-green-500/70 transition-all"
                          style={{ width: total > 0 ? `${(selfFundedCount / total) * 100}%` : '0%' }}
                        />
                      </div>
                    </div>
                  );
          ...
          {/* No members on trip */}
          {costItemsLoaded && costItems.length > 0 && tripMembers.length === 0 && (
            <div className="rounded-xl border border-brand-tan/15 bg-brand-dark-grey p-8 text-center text-brand-cream/50 text-sm">
              No members found for this trip. Add members in the trip settings first.
            </div>
          )}
        </div>
      )}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          EXPORT TAB — ATO-compliant tax records
      ══════════════════════════════════════════════════════════════════════ */}
      {tab === 'export' && (() => {
        // Derive unique financial years from all transactions
        const allDates = [
          ...expenses.map((e) => e.expense_date),
          ...incomeEntries.map((e) => e.income_date),
        ];
        const fySet = new Set<string>();
        allDates.forEach((d) => {
          const dt = parseLocalDate(d);
          const fy = dt.getMonth() >= 6 ? dt.getFullYear() : dt.getFullYear() - 1;
          fySet.add(String(fy));
        });
        const fyOptions = Array.from(fySet).sort((a, b) => Number(b) - Number(a));

        // Helper: check if a date falls in a given financial year (Jul–Jun)
        const inFY = (dateStr: string, fy: string) => {
          if (!fy) return true;
          const dt = parseLocalDate(dateStr);
          const year = Number(fy);
          return dt >= new Date(year, 6, 1) && dt < new Date(year + 1, 6, 1);
        };

        // GST helper — trip expenses are personal/club travel, generally not GST-registered
        // Include GST column as 'N/A' unless category implies otherwise
        const gstLabel = (e: Expense) =>
          e.currency !== 'AUD' ? 'Foreign (no GST)' : 'N/A – personal/club';

        // Payer label helper
        const payerLabel = (e: Expense) =>
          e.paid_by_type === 'group_kitty' ? 'Group Kitty'
            : e.paid_by_type === 'member' ? (e.payer?.nickname || e.payer?.full_name || 'Member')
            : (e.paid_by_label || 'External');

        // ── Export functions ──────────────────────────────────────────────

        const exportTransactionRegister = (fy: string) => {
          const filteredExp = expenses.filter((e) => !fy || inFY(e.expense_date, fy));
          const filteredInc = incomeEntries.filter((e) => !fy || inFY(e.income_date, fy));
          const rows: string[][] = [
            ['ATO Transaction Register — Whiskey Riders Morocco 2027'],
            [fy ? `Financial Year: ${fy}–${Number(fy)+1}` : 'All Years'],
            ['Generated:', new Date().toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })],
            ['Record retention required: 5 years from date of transaction (ATO s262A ITAA 1936)'],
            [],
            ['Date', 'Type', 'Description', 'Category', 'Account', 'Currency', 'Original Amount', 'AUD Amount', 'Paid By / Source', 'GST Treatment', 'Notes'],
          ];
          // Income rows
          filteredInc.forEach((e) => {
            rows.push([
              fmtDate(e.income_date), 'Income',
              e.description,
              e.category || 'Other',
              getAccountDisplayName(parseTransactionNote(e.notes).account_source_id),
              'AUD', String(e.amount_aud), String(e.amount_aud),
              'Member/Club Income',
              'N/A – personal/club',
              parseTransactionNote(e.notes).text || '',
            ]);
          });
          // Expense rows
          filteredExp.forEach((e) => {
            rows.push([
              fmtDate(e.expense_date), 'Expense',
              e.description,
              e.category?.name || 'Uncategorised',
              getAccountDisplayName(parseTransactionNote(e.notes).account_source_id),
              e.currency,
              String(Math.abs(e.amount || e.amount_aud)),
              String(Math.abs(e.amount_aud)),
              payerLabel(e),
              gstLabel(e),
              parseTransactionNote(e.notes).text || '',
            ]);
          ...
          // Totals
          const totalInc = filteredInc.reduce((s, e) => s + e.amount_aud, 0);
          const totalExp = filteredExp.reduce((s, e) => s + e.amount_aud, 0);
          rows.push([], ['', '', '', '', '', '', '', '', '', ''],
            ['TOTALS', '', '', '', '', '', '', '', '', ''],
            ['Total Income (AUD)', '', '', '', '', '', '', String(totalInc), '', '', ''],
            ['Total Expenses (AUD)', '', '', '', '', '', '', String(Math.abs(totalExp)), '', '', ''],
            ['Net Position (AUD)', '', '', '', '', '', '', String(totalInc + totalExp), '', '', ''],
          );
          const fyStr = fy ? `FY${fy}-${Number(fy)+1}` : 'AllYears';
          downloadCsv(`WR-Morocco2027-TransactionRegister-${fyStr}.csv`, rows);
        };

        const exportExpenseSummary = (fy: string) => {
          const filtered = expenses.filter((e) => !fy || inFY(e.expense_date, fy));
          const byCat: Record<string, { total: number; count: number; items: Expense[] }> = {};
          filtered.forEach((e) => {
            const cat = e.category?.name || 'Uncategorised';
            if (!byCat[cat]) byCat[cat] = { total: 0, count: 0, items: [] };
            byCat[cat].total += Math.abs(e.amount_aud);
            byCat[cat].count++;
            byCat[cat].items.push(e);
          });
          const rows: string[][] = [
            ['ATO Expense Summary by Category — Whiskey Riders Morocco 2027'],
            [fy ? `Financial Year: ${fy}–${Number(fy)+1}` : 'All Years'],
            ['Generated:', new Date().toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })],
            [],
            ['Category', 'Number of Transactions', 'Total AUD', 'GST Treatment'],
          ];
          Object.entries(byCat)
            .sort(([, a], [, b]) => b.total - a.total)
            .forEach(([cat, { total, count }]) => {
              rows.push([cat, String(count), String(total.toFixed(2)), 'N/A – personal/club travel']);
            });
          const grandTotal = filtered.reduce((s, e) => s + Math.abs(e.amount_aud), 0);
          rows.push([], ['TOTAL', String(filtered.length), String(grandTotal.toFixed(2)), '']);
          rows.push([]);
          rows.push(['Detailed Transactions']);
          rows.push(['Date', 'Category', 'Description', 'Paid By', 'Currency', 'Original', 'AUD', 'Account', 'Notes']);
          filtered
          ...
          const fyStr = fy ? `FY${fy}-${Number(fy)+1}` : 'AllYears';
          downloadCsv(`WR-Morocco2027-ExpenseSummary-${fyStr}.csv`, rows);
        };

        const exportMemberCollections = (fy: string) => {
          const filtered = incomeEntries.filter((e) =>
            e.category !== 'interest' && (!fy || inFY(e.income_date, fy))
          );
          const rows: string[][] = [
            ['ATO Income / Member Collections — Whiskey Riders Morocco 2027'],
            [fy ? `Financial Year: ${fy}–${Number(fy)+1}` : 'All Years'],
            ['Generated:', new Date().toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })],
            ['Note: Member contributions to a shared trip kitty are not assessable income for the trip organiser'],
            ['       and are not deductible for members. Records kept for 5 years per ATO s262A ITAA 1936.'],
            [],
            ['Date', 'Description', 'Category', 'Amount AUD', 'Account', 'Notes'],
          ];
          filtered
            .sort((a, b) => a.income_date.localeCompare(b.income_date))
            .forEach((e) => {
              rows.push([
                fmtDate(e.income_date),
                e.description,
                e.category || 'Member Payment',
                String(e.amount_aud),
                getAccountDisplayName(parseTransactionNote(e.notes).account_source_id),
                parseTransactionNote(e.notes).text || '',
              ]);
            });
          const total = filtered.reduce((s, e) => s + e.amount_aud, 0);
          rows.push([], ['TOTAL', '', '', String(total.toFixed(2)), '', '']);
          const fyStr = fy ? `FY${fy}-${Number(fy)+1}` : 'AllYears';
          downloadCsv(`WR-Morocco2027-MemberCollections-${fyStr}.csv`, rows);
        };

        const exportFinancialYearSummary = () => {
          const rows: string[][] = [
            ['ATO Financial Year Summary — Whiskey Riders Morocco 2027'],
            ['Generated:', new Date().toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })],
            ['Record retention: 5 years per ATO s262A ITAA 1936 | Records in English ✓ | Exportable as CSV ✓'],
            [],
            ['Financial Year', 'Income (AUD)', 'Expenses (AUD)', 'Net Position (AUD)', 'Transaction Count'],
          ];
          const fyData: Record<string, { inc: number; exp: number; count: number }> = {};
          incomeEntries.forEach((e) => {
            const dt = parseLocalDate(e.income_date);
            const fy = dt.getMonth() >= 6 ? `${dt.getFullYear()}–${dt.getFullYear()+1}` : `${dt.getFullYear()-1}–${dt.getFullYear()}`;
            if (!fyData[fy]) fyData[fy] = { inc: 0, exp: 0, count: 0 };
            fyData[fy].inc += e.amount_aud;
            fyData[fy].count++;
          });
          expenses.forEach((e) => {
            ...
            const fy = dt.getMonth() >= 6 ? `${dt.getFullYear()}–${dt.getFullYear()+1}` : `${dt.getFullYear()-1}–${dt.getFullYear()}`;
            if (!fyData[fy]) fyData[fy] = { inc: 0, exp: 0, count: 0 };
            fyData[fy].exp += Math.abs(e.amount_aud);
            fyData[fy].count++;
          });
          Object.entries(fyData)
            .sort(([a], [b]) => a.localeCompare(b))
            .forEach(([fy, { inc, exp, count }]) => {
              rows.push([fy, String(inc.toFixed(2)), String(exp.toFixed(2)), String((inc - exp).toFixed(2)), String(count)]);
            });
          const totalInc = incomeEntries.reduce((s, e) => s + e.amount_aud, 0);
          const totalExp = expenses.reduce((s, e) => s + Math.abs(e.amount_aud), 0);
          rows.push(['TOTAL (all years)', String(totalInc.toFixed(2)), String(totalExp.toFixed(2)), String((totalInc - totalExp).toFixed(2)), String(incomeEntries.length + expenses.length)]);
          downloadCsv('WR-Morocco2027-FinancialYearSummary.csv', rows);
        };

        return (
          <div className="space-y-6">
            {/* ATO info banner */}
            <div className="flex items-start gap-3 p-4 bg-brand-dark-grey border border-brand-tan/20 rounded-xl">
              <FileText className="w-5 h-5 text-brand-tan mt-0.5 flex-shrink-0" />
              <div className="space-y-1">
                <p className="font-semibold text-brand-cream">ATO Record-Keeping Compliance</p>
                <p className="text-sm text-brand-cream/60">
                  Australian tax law (ITAA 1936 s262A) requires financial records to be kept for <strong className="text-brand-cream/80">5 years</strong> from the date of transaction. Records must include date, amount, description, and purpose. All exports are in English and CSV format as required by the ATO.
                </p>
                <p className="text-xs text-brand-cream/40 mt-1">
                  Note: Member contributions to a shared trip kitty are generally not assessable income for the organiser and not deductible for members. Consult your tax agent if any member is claiming trip costs as a work-related deduction.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
                  <label className="text-sm text-brand-cream/60 font-medium">Filter by financial year:</label>
                  <select
                    value={exportFY}
                    onChange={(e) => setExportFY(e.target.value)}
                    className="px-3 py-2 bg-brand-black border border-brand-tan/30 rounded-lg text-brand-cream text-sm focus:outline-none focus:ring-2 focus:ring-brand-tan"
                  >
                    <option value="">All years</option>
                    {fyOptions.map((fy) => (
                      <option key={fy} value={fy}>{fy}–{Number(fy)+1}</option>
                    ))}
                  </select>
                  <span className="text-xs text-brand-cream/30">(Australian financial year: 1 Jul – 30 Jun)</span>

                  <div className="ml-auto flex gap-2 flex-wrap">
                    {/* Full data export */}
                    <button
                      onClick={() => exportTransactionRegister(exportFY)}
                      className="flex items-center gap-2 bg-brand-tan hover:bg-brand-tan/90 text-brand-black px-4 py-2 rounded-lg text-sm font-semibold"
                    >
                      <Download className="w-4 h-4" /> Full Register
                    </button>
                    <button
                      onClick={exportFinancialYearSummary}
                      className="flex items-center gap-2 border border-brand-tan/40 hover:border-brand-tan text-brand-cream px-4 py-2 rounded-lg text-sm font-semibold hover:bg-brand-tan/10"
                    >
                      <Download className="w-4 h-4" /> FY Summary
                    </button>
                  </div>
                </div>
            ...
            {/* Export cards */}
            {(() => {
              const exportFY2 = exportFY;
              // Computed stats for the selected FY
              const filteredExp = expenses.filter((e) => !exportFY2 || (() => {
                const dt = parseLocalDate(e.expense_date);
                const fy = dt.getMonth() >= 6 ? dt.getFullYear() : dt.getFullYear() - 1;
                return String(fy) === exportFY2;
              })());
              const filteredInc = incomeEntries.filter((e) => !exportFY2 || (() => {
                const dt = parseLocalDate(e.income_date);
                const fy = dt.getMonth() >= 6 ? dt.getFullYear() : dt.getFullYear() - 1;
                return String(fy) === exportFY2;
              })());
              const totalInc = filteredInc.reduce((s, e) => s + e.amount_aud, 0);
              const totalExp = filteredExp.reduce((s, e) => s + Math.abs(e.amount_aud), 0);
              const uniqueCategories = new Set(filteredExp.map((e) => e.category?.name || 'Uncategorised')).size;

              const cards = [
                {
                  title: 'Transaction Register',
                  subtitle: 'ATO s262A compliant — full record of all income and expenses',
                  fields: ['Date', 'Type (Income/Expense)', 'Description & purpose', 'Category', 'Account', 'Currency & amount', 'AUD equivalent', 'GST treatment', 'Notes'],
                  stats: `${filteredInc.length + filteredExp.length} transactions · ${fmt(totalInc)} income · ${fmt(totalExp)} expenses`,
                  action: () => exportTransactionRegister(exportFY2),
                  label: 'Export CSV',
                  color: 'border-brand-tan/30',
                },
                {
                  title: 'Expense Summary by Category',
                  subtitle: 'Itemised expenses grouped by category with totals',
                  fields: ['Category breakdown', 'Transaction count per category', 'Total AUD per category', 'Full line-item detail', 'GST treatment per line'],
                  stats: `${filteredExp.length} expenses across ${uniqueCategories} categories · ${fmt(totalExp)} total`,
                  action: () => exportExpenseSummary(exportFY2),
                  label: 'Export CSV',
                  color: 'border-red-600/20',
                },
                {
                  title: 'Member Collections Report',
                  subtitle: 'Income received from members — includes ATO note on tax treatment',
                  fields: ['Date received', 'Member/description', 'Category (instalment, deposit, etc.)', 'Amount AUD', 'Account received into', 'Running total'],
                  stats: `${filteredInc.length} income entries · ${fmt(totalInc)} total collected`,
                  action: () => exportMemberCollections(exportFY2),
                  label: 'Export CSV',
                  color: 'border-green-600/20',
                },
                {
                  title: 'Financial Year Summary',
                  subtitle: 'Year-by-year income/expense totals across all trip years',
                  fields: ['One row per financial year (Jul–Jun)', 'Total income AUD', 'Total expenses AUD', 'Net position', 'Transaction count'],
                  stats: `Covers ${fyOptions.length} financial ${fyOptions.length === 1 ? 'year' : 'years'}`,
                  action: exportFinancialYearSummary,
                  label: 'Export CSV',
              ...
              return (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {cards.map(({ title, subtitle, fields, stats, action, label, color }) => (
                    <div key={title} className={`bg-brand-dark-grey border ${color} rounded-xl p-5 flex flex-col gap-4`}>
                      <div>
                        <h3 className="font-semibold text-brand-cream">{title}</h3>
                        <p className="text-xs text-brand-cream/50 mt-0.5">{subtitle}</p>
                      </div>
                      <div>
                        <p className="text-[11px] uppercase tracking-wide text-brand-cream/30 mb-1.5">Includes</p>
                        <ul className="space-y-0.5">
                          {fields.map((f) => (
                            <li key={f} className="flex items-center gap-1.5 text-xs text-brand-cream/60">
                              <span className="w-1 h-1 rounded-full bg-brand-tan/50 flex-shrink-0" />
                              {f}
                            </li>
                          ))}
                        </ul>
                      </div>
                      <div className="mt-auto pt-3 border-t border-brand-tan/10 flex items-center justify-between">
                        <p className="text-xs text-brand-cream/40">{stats}</p>
                        <button
                          onClick={action}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-tan/20 border border-brand-tan/40 text-brand-tan rounded-lg text-xs font-semibold hover:bg-brand-tan/30 transition-colors"
                        >
            ...
            {/* ATO retention reminder */}
            <div className="flex items-start gap-3 p-4 bg-brand-black/40 border border-brand-tan/10 rounded-xl">
              <Shield className="w-4 h-4 text-brand-cream/30 mt-0.5 flex-shrink-0" />
              <div className="text-xs text-brand-cream/40 space-y-1">
                <p><span className="text-brand-cream/60 font-medium">5-year retention rule:</span> Keep exported records until at least 5 years after the trip concludes (approx. June 2032 for Morocco 2027).</p>
                <p><span className="text-brand-cream/60 font-medium">Format:</span> CSV files can be opened in Excel, Google Sheets, or Numbers and are in the ATO-required English language format.</p>
                <p><span className="text-brand-cream/60 font-medium">GST:</span> This trip is run as a personal/club activity, not a GST-registered business. No GST input tax credits are claimable on these expenses. If your situation differs, consult your BAS agent.</p>
              </div>
            </div>
          </div>
        );
            ...
            <div>
              ...
              {interestRatePa > 0 && (
                <p className="text-xs text-amber-400/70 mt-1">
                  Projected monthly interest: ~{fmt((accountFlowSummary.find((r) => r.id === 'westpac_life')?.net ?? 0) * (interestRatePa / 100) / 12)} at ledger balance
                </p>
              )}
            </div>
